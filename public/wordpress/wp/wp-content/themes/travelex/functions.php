...
function travelex_setup() {
	
	//Register a theme image size.
	add_theme_support( 'post-thumbnails' );
	add_image_size( 'travelex-large-thumb', 830 );
	add_image_size( 'travelex-medium-thumb', 550, 400, true );
	add_image_size( 'travelex-small-thumb', 230 );
	
	//Register nav menu used in theme.
	register_nav_menus( array(
		'primary' 	=> __( 'Primary Menu', 'travelex' )
	) );
...
add_action( 'after_setup_theme', 'travelex_setup' );


/*--- Theme preloader ---*/
function travelex_preloader() {
	?>
	<div class="preloader">
	    <div class="spinner">
...
add_action('travelex_before_site', 'travelex_preloader');
...
function travelex_grid_post_meta(){
...
function travelex_excerpt_more($more) {
	global $post;
	$layout = theme_blog_layout();
	return ' ....&nbsp;&nbsp;<a class=" '.$layout.'" href="'. get_permalink($post->ID) . '">' . __('Read More', 'travelex') . '</a>';
}
add_filter('excerpt_more', 'travelex_excerpt_more');


/*--- Enqueue scripts and styles ---*/
function travelex_scripts() {
	...
	wp_enqueue_script( 'travelex-common', get_template_directory_uri() . '/js/common.js', array('jquery'), '20180213', true );
	...
	wp_enqueue_style( 'travelex-font-awesome', get_template_directory_uri() . '/fonts/font-awesome.min.css' );

	if ( is_singular() && comments_open() && get_option( 'thread_comments' ) ) {
		wp_enqueue_script( 'comment-reply' );
	}
	
	/* wp_enqueue_style( 'travelex-bootstrap', get_template_directory_uri() . '/css/bootstrap/bootstrap.min.css', array(), true ); */
	
}
add_action( 'wp_enqueue_scripts', 'travelex_scripts' );

/**
 * Enqueue Bootstrap
 */
function travelex_enqueue_bootstrap() {
	wp_enqueue_style( 'travelex-bootstrap', get_template_directory_uri() . '/css/bootstrap/bootstrap.min.css', array(), true );
}
add_action( 'wp_enqueue_scripts', 'travelex_enqueue_bootstrap', 9 );



/*--- Register widget area ---*/
add_action( 'widgets_init', 'travelex_widgets_init' );
function travelex_widgets_init() {
    register_sidebar( array(
        'name' => __( 'Main Sidebar', 'travelex' ),
        'id' => 'sidebar-1',
	...
	for ($i=1; $i<=3; $i++){
		register_sidebar( array(
			'name' => __( 'Footer-widget-area-', 'travelex' ) . $i,
...
function travelex_edit_link() {
	edit_post_link(
		sprintf( __( '<i class="fa fa-edit"></i>Edit<span class="screen-reader-text"> "%s"</span>', 'travelex' ), get_the_title() ),
...
function travelex_get_breadcrumb() {
    echo '<a href="'.home_url().'" rel="nofollow">'.__('Home', 'travelex').'</a>';
    if ( is_category() || is_single() ) {
        ...
        echo __("&nbsp;&nbsp;&#47;&nbsp;&nbsp;Search Results for... ", "travelex");
        echo '"<em>';
...
function bizworx_grid_post_meta(){
...
function travelex_recommended_plugin() {
...
add_action( 'tgmpa_register', 'travelex_recommended_plugin' );
...
add_filter( 'pre_set_site_transient_update_themes', 'travelex_check_for_update' );
function travelex_check_for_update( $checked_data ){
...
popularfx_maybe_promo([
        'after' => 1,// In days
        'interval' => 30,// In days
        'pro_url' => 'https://popularfx.com/themes/wordpress/corporate/Travelex_Pro',
        'rating' => 'https://popularfx.com/themes/wordpress/corporate/Travelex',
        'twitter' => 'https://twitter.com/theme_worx?status='.rawurlencode('I love #Travelex theme by @theme_worx for my #WordPress site - '.home_url()),
        'facebook' => 'https://www.facebook.com/themeworxofficial',
        'website' => 'http://themeworx.net',
]);

function travelex_upsell_script() {
...
add_action( 'customize_controls_enqueue_scripts', 'travelex_upsell_script' );
