...
<html <?php language_attributes(); ?>>
  ...
  <?php do_action('travelex_before_site'); 
  $menu_type = '';
  if( function_exists(profism_sticky_header) ){
	$menu_type = profism_sticky_header();
  }?>
  	<?php 
	if ( function_exists( 'travelex_contact_info' ) ) {
		travelex_contact_info(); 
	}?>
			...
			<div class="header-background other-header">
				<div class="header-content other">
					...
					<div class = "breadcrumb" ><?php if (class_exists('WooCommerce')){
							( (is_woocommerce())? woocommerce_breadcrumb() : travelex_get_breadcrumb() );
						}else{
							travelex_get_breadcrumb();
						}?></div>
					<?php endif; ?>
				</div>
			</div>
